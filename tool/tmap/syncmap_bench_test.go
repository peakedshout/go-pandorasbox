...
type bench struct {
	setup func(*testing.B, Map)
	perG  func(b *testing.B, pb *testing.PB, i int, m Map)
}

func benchMap(b *testing.B, bench bench) {
	for _, m := range [...]Map{&DeepCopyMap{}, &RWMutexMap{}, &sync.Map{}, NewTSyncMapPtr[any, any](), &SyncMap[any, any]{}} {
		b.Run(fmt.Sprintf("%T", m), func(b *testing.B) {
			m = reflect.New(reflect.TypeOf(m).Elem()).Interface().(Map)
			if bench.setup != nil {
				bench.setup(b, m)
			}
