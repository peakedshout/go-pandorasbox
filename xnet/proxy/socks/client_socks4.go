...
func newSocks4Config(network string, address string, cd byte, userId S4UserId, forward xnetutil.Dialer, bindCb BINDAddrCb) (*socks4Config, error) {
	return &socks4Config{
...
func (s4d *socks4Config) DialContext(ctx context.Context, network string, addr string) (c net.Conn, err error) {
	...
	if err != nil || ctxtool.Disable(ctx) {
		_ = conn.Close()
		return nil, err
	}
	return conn, nil
